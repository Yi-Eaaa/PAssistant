...
Use passistant programmatically — load config, run the agent, get results.

## Quick Start

```python
import asyncio
from passistant import passistant
...
### `passistant.from_config(config_path?, *, workspace?)`

Create a `passistant` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.passistant/config.json`. |
...
```python
from passistant.agent import AgentHook, AgentHookContext
...
async def main():
    bot = passistant.from_config(workspace="/my/project")
