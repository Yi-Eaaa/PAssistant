...
Build a custom passistant channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of passistant (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

passistant discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `passistant gateway` starts, it scans:

1. Built-in channels in `passistant/channels/`
2. External packages registered under the `passistant.channels` entry point group
...
```
passistant-channel-webhook/
├── passistant_channel_webhook/
...
```python
# passistant_channel_webhook/__init__.py
from passistant_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# passistant_channel_webhook/channel.py
import asyncio
...
from passistant.channels.base import BaseChannel
...
# pyproject.toml
[project]
name = "passistant-channel-webhook"
version = "0.1.0"
dependencies = ["passistant", "aiohttp"]

[project.entry-points."passistant.channels"]
webhook = "passistant_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
passistant plugins list      # verify "Webhook" shows as "plugin"
passistant onboard           # auto-adds default config for detected plugins
```

Edit `~/.passistant/config.json`:
...
```bash
passistant channels login <channel_name>
passistant channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `passistant onboard`. Override to declare your fields. |
...
Override `default_config()` so `passistant onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `passistant-channel-{name}` | `passistant-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `passistant_channel_{name}` | `passistant_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/passistant-channel-webhook
cd passistant-channel-webhook
pip install -e .
passistant plugins list    # should show "Webhook" as "plugin"
passistant gateway         # test end-to-end
```

## Verify

```bash
$ passistant plugins list
