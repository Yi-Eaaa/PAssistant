...
try:
    from passistant.channels import dingtalk
...
import passistant.channels.dingtalk as dingtalk_module
from passistant.channels.dingtalk import DingTalkChannel, passistantDingTalkHandler
from passistant.channels.dingtalk import DingTalkConfig
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "passistant Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = passistantDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/passistant_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/passistant_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "passistant.config.paths.get_media_dir",
