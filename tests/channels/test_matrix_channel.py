...
from passistant.channels.matrix import _build_matrix_text_content

import passistant.channels.matrix as matrix_module
from passistant.bus.events import OutboundMessage
from passistant.bus.queue import MessageBus
from passistant.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from passistant.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("passistant.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "passistant.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("passistant.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "passistant.channels.matrix.asyncio.create_task", _fake_create_task
    )
